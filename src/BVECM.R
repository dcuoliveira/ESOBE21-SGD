...
data_ex_sneer = data %>% select(-SNEER)
ts_data = ts(data = data_ex_sneer, 
             freq = 52, 
             start = decimal_date(ymd(rownames(data)[1])))
data = gen_vec(data = ts_data,
               p = 1, # lag order of the VAR form of the model
